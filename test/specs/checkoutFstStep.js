...
it('should deny sending the form when providing numbers for first and last name', () => {
    ...
    expect(checkoutFstStepPage.errorMsgContainer).toBeDisplayed
    browser.pause(3000)
    //THE FORM IS PROPERLY SENT EVEN THOUGH A VALID NAME AND LAST NAME ARE NOT PROVIDED. NO ERROR MESSAGE SHOWN-
    })
it('should deny sending the form when providing a valid name and last name and an invalid zip code', () => {
    ...
    expect(checkoutFstStepPage.errorMsgContainer).toBeDisplayed
    browser.pause(3000)
    //THE FORM IS PROPERLY SENT EVEN THOUGH AN INVALID ZIP CODE IS PROVIDED. NO ERROR MESSAGE SHOWN-
    })
it('should deny sending the form when providing special characters as imputs', () => {
    ...
    expect(checkoutFstStepPage.errorMsgContainer).toBeDisplayed
    browser.pause(3000)
    //THE FORM IS SENT WHEN FILLING IN THE BLANKS WITH SPECIAL CHARACTERS AS IMPUTS. NO ERROR MESSAGE SHOWN-
    })
it('should deny sending the form when providing valid imputs, but leaving a blank space before each of them', () => {
    ...
    expect(checkoutFstStepPage.errorMsgContainer).toBeDisplayed
    browser.pause(3000)
    //THE FORM IS SENT WHEN LEAVING A BLANK SPACE BEFORE THE IMPUTS. NO ERROR MESSAGE SHOWN-
    })
});